...
def main():
    # Resolve repo root so icons are always written to repo root, not scripts/
    ...
    if not os.path.isdir(os.path.join(repo_root, "app")):
        repo_root = here
    icon_png = os.path.join(repo_root, "icon.png")
    icon_ico = os.path.join(repo_root, "icon.ico")

    big = make_logo(1024)
    big.resize((512, 512), Image.LANCZOS).save(icon_png)
    # All the sizes Windows asks for (256 gets PNG-compressed automatically)
    sizes = [(16,16),(20,20),(24,24),(32,32),(40,40),(48,48),(64,64),(128,128),(256,256)]
    big.save(icon_ico, sizes=sizes)
    # sanity check
    if os.path.exists(icon_ico) and os.path.getsize(icon_ico) > 1000:
        print("✓ icon.ico created (%d bytes)" % os.path.getsize(icon_ico))
    else:
