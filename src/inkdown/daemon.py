...
# Repo root is two levels up from src/inkdown/; fallback to SCRIPT_DIR for frozen/legacy
...
def open_main_app():
    ...
    try:
        # Prefer repo-root shim (works with src layout); fall back to package run
        ...
        if main_py.exists():
            ...
            log("Launching main app via -m inkdown...")
            subprocess.Popen(
                [sys.executable, "-m", "inkdown"],
                cwd=str(REPO_ROOT),
                creationflags=subprocess.CREATE_NEW_CONSOLE
            )
        return
...
def open_capture():
    ...
    try:
        ...
        if main_py.exists():
            ...
            log("Launching Quick Capture via -m inkdown --capture...")
            subprocess.Popen(
                [sys.executable, "-m", "inkdown", "--capture"],
                cwd=str(REPO_ROOT),
                creationflags=subprocess.CREATE_NEW_CONSOLE
            )
        return
